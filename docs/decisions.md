...
- **Source:** GT new_pools polled every 2–3 min at poll priority (~0.5 calls/min — cheap even on the shared IP), plus one token lookup per Uniswap candidate. On-chain PoolCreated/Initialize events over a Robinhood Chain RPC are the zero-GT, seconds-latency upgrade path once an RPC is verified.
