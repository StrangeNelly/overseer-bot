...
- 2026-09-01: M0+M1 built (npm-workspaces monorepo: packages/shared, packages/db, apps/server) and hardened via 19-agent adversarial review (13 confirmed findings fixed — see commit 1f645a2). Typecheck + 9 tests green; migration 0000 generated, NOT yet applied (needs DATABASE_URL). Awaiting owner: BotFather token + Supabase connection string, then live scratch-group test.
