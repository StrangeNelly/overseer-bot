...
import { calls, type Db } from '@groupie/db';
 ...
 */
function changesRelevance(event: GroupieEvent): boolean {
  return event.type === 'new_call' || event.type === 'call_revived';
}
...
async function isGroupToken(db: Db, groupId: number, tokenId: number): Promise<boolean> {
  ...
  const found = await db
    .select({ id: calls.id })
    .from(calls)
    .where(and(eq(calls.groupId, groupId), eq(calls.tokenId, tokenId)))
    .limit(1);
    ...
    return streamSSE(c, async (stream) => {
      ...
      unsubscribe = subscribe((event: GroupieEvent) => {
        ...
        enqueue(async () => {
          ...
          // token relevant for every group with an open stream.
          if (changesRelevance(event)) relevance.delete(`${group.id}:${event.tokenId}`);
          if (!(await isGroupToken(db, group.id, event.tokenId))) return;
